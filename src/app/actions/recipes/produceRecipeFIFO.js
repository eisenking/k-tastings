...
export async function produceRecipeFIFO({ recipeId, amountBase, expirationDate = null }) {
    ...
    const targetAmount = toNum(amountBase, 0);
    if (!recipeId || targetAmount <= 0) throw new Error("recipeId/amountBase required");
    ...
    return await db.transaction(async (tx) => {
        ...
        async function getItems(recipeIdInner) {
            ...
            return rows.map((x) => ({ ...x, amountBaseNum: toNum(x.amountBase, 0) }));
        }
        ...
        async function getProductBatchesWithRemaining(productId) {
            // партии товара FIFO — по времени создания партии
            const batches = await tx
                .select({
                    id: ProductBatchesTable.id,
                ...
                .from(ProductBatchesTable)
                .where(eq(ProductBatchesTable.productId, productId))
                .orderBy(asc(ProductBatchesTable.createdAt));

            if (batches.length === 0) return [];

            const batchIds = batches.map((b) => b.id);

            // остаток партии по движениям: SUM(Приход) - SUM(Списание) - SUM(Производство)
            const moves = await tx
                .select({
                    batchId: StockMovementsTable.batchId,
                    remainingBase: sql`
            ...
            const remMap = new Map(moves.map((m) => [m.batchId, toNum(m.remainingBase, 0)]));

            return batches.map((b) => ({
                ...b,
                remainingBase: remMap.get(b.id) ?? 0,
                unitCostBaseNum: toNum(b.unitCostBase, 0),
            }));
        }
        ...
        async function consumeProductFIFO({ productId, requiredBase, reason, targetBatchId }) {
            ...
            for (const b of batches) {
                ...
                const takeBase = Math.min(avail, need);
                const cost = takeBase * b.unitCostBaseNum;

                // движение склада (списание на производство)
                await tx.insert(StockMovementsTable).values({
                    productId,
                    batchId: b.id,
                    type: "Производство",
                ...
                // лог потребления для партии производства
                await tx.insert(ProductionConsumptionsTable).values({
                    targetBatchId,
                    sourceType: "product_batch",
                    productBatchId: b.id,
            ...
            if (need > 0.000001) {
                ...
                throw new Error(`Недостаточно "${p?.name || "продукта"}": не хватает ${Math.ceil(need)} (base)`);
            }
        ...
        async function consumePreparationFIFO({ prepRecipeId, requiredBase, reason, targetBatchId }) {
            ...
            for (const b of batches) {
                ...
                await tx
                    .update(ProductionBatchesTable)
                    .set({ remainingBase: String(round6(avail - takeBase)) })
                    .where(eq(ProductionBatchesTable.id, b.id));
        ...
        async function produceInternal(recipeIdInner, amountBaseInner, expirationDateInner) {
            if (visited.has(recipeIdInner)) throw new Error("Циклическая ссылка в рецептах (обнаружен цикл)");
            visited.add(recipeIdInner);

            const recipe = await getRecipe(recipeIdInner);
            const items = await getItems(recipeIdInner);
            if (items.length === 0) throw new Error(`У рецепта "${recipe.name}" нет состава`);
            ...
            const [targetBatch] = await tx
                ...
                .values({
                    recipeId: recipe.id,
                    producedBase: String(round6(amountBaseInner)),
                    remainingBase: String(round6(amountBaseInner)),
                    totalCost: "0",
                    unitCostBase: "0",
                    producedAt: new Date(),
                    expirationDate: expirationDateInner,
        ...
        return await produceInternal(recipeId, targetAmount, expirationDate);
    });
}
