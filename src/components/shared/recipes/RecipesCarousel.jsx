...
import { PREP_TAB_LABELS, prepCategoryFromLabel } from "@/lib/helpers/recipeCategories";
...
export default function RecipesCarousel({
	...
	async function fetchRecipes(nextType = type, nextCat = prepCat) {
		...
		try {
			const category =
				nextType === "preparation" ? prepCategoryFromLabel(nextCat) : null;

			const res = await getRecipesByType({
				location,
				type: nextType,
				category,
			});
			...
			if (!res?.ok) {
				toast.error(res?.error || "Не удалось загрузить техкарты");
				setRecipes([]);
				return;
			}
	...
	const defaultPrepCategory =
		type === "preparation" ? prepCategoryFromLabel(prepCat) : null;
		...
		<div className="flex flex-col gap-2 ">
			...
			<div className="flex flex-col justify-center items-center gap-2">
				<AddRecipeForm
					...
					defaultPrepCategory={defaultPrepCategory}
				/>
				{type === "preparation" && (
							...
							>
								<CardContent className="flex aspect-square items-center justify-center p-6">
									<div className="text-center">
										...
										{type === "preparation" && recipe.usedInParents?.length > 0 && (
											<div className="flex flex-wrap gap-1 justify-center mt-2">
												{recipe.usedInParents.map((parent) => (
												<Badge
													key={parent}
													variant="secondary"
