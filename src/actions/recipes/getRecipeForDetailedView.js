...
// actions/recipes/getRecipeForDetailedView.js
"use server";
...
export const getRecipeForDetailedView = withAction(async ({ recipeId } = {}) => {
    // 1. Auth
    const user = await requireUser();

    // 4. Business logic (RBAC после загрузки — нужна локация)
    // 4.1 Загружаем рецепт
    const [recipe] = await db
        .select({
            id: RecipesTable.id,
            name: RecipesTable.name,
    ...
    // 4.2 Items + связанные продукты/подрецепты
    const items = await db
        .select({
            id: RecipeItemsTable.id,
            refType: RecipeItemsTable.refType,
    ...
    // 4.3 Доступность продуктов — sum(remaining_base) по партиям в локации рецепта
    const productIds = items
        .filter((i) => i.refType === "product" && i.productId)
        .map((i) => i.productId);

    const productAvailMap = new Map();
    if (productIds.length > 0) {
        const rows = await db
            .select({
            ...
            .where(
                and(
                    ...
                    eq(ProductBatchesTable.location, recipe.location),
                ),
            )
            .groupBy(ProductBatchesTable.productId);
        for (const r of rows) productAvailMap.set(r.productId, r.availableBase);
    }

    // 4.4 Доступность заготовок — sum(remaining_base) по production_batches
    const childIds = items
        .filter((i) => i.refType === "recipe" && i.childRecipeId)
        .map((i) => i.childRecipeId);

    const recipeAvailMap = new Map();
    if (childIds.length > 0) {
        const rows = await db
            ...
            .where(
                and(
                    inArray(ProductionBatchesTable.recipeId, childIds),
                    eq(ProductionBatchesTable.location, recipe.location),
                ),
            )
            .groupBy(ProductionBatchesTable.recipeId);
        for (const r of rows) recipeAvailMap.set(r.recipeId, r.availableBase);
    }
    ...
    const normalized = items.map((i) => ({
        ...
        childRecipe:
            i.refType === "recipe"
                ? { id: i.childRecipeId, name: i.childRecipeName, type: i.childRecipeType }
                : null,
    }));
